...
- **Status:** `accepted` — Fase 2.
- **Konteks:** skema sebelumnya dibuat `Base.metadata.create_all` di lifespan FastAPI — aman untuk dev/tests, tapi di produksi: (1) tidak bisa menghapus/mengubah kolom (create_all hanya ADD), (2) tanpa versioning, setiap rilis fitur (FR-07/08/09/11) memaksa `docker compose down -v` (data dev hilang) atau ALTER manual di VPS.
- **Keputusan:**
  - **Alembic** ditambahkan sebagai dependency runtime (`api/migrations/`, `alembic.ini`). `env.py` memakai URL dari `settings.database_url` (satu sumber kebenaran dengan engine runtime) dan meng-import SEMUA model (termasuk `Transaction` — kini ikut di `models/__init__.py`).
  - **`Base.metadata.create_all` DIHAPUS dari runtime** (main.py lifespan). Test tetap memanggil `create_all` eksplisit per-fixture (SQLite in-memory) — tidak berubah.
  - **Migrasi awal `0001_initial`** = skema HEAD (users/jobs/transactions + seluruh kolom FR). Dirancang **konvergen**: guard per tabel + **backfill kolom** (dengan `server_default` untuk baris lama) sehingga DB fresh MAUPUN DB lama era `create_all` (VPS) naik ke `head` tanpa error dan tanpa kehilangan data.
  - **Integrasi deploy**: service `api` di docker-compose menjalankan `alembic upgrade head` otomatis sebelum uvicorn start → upgrade di VPS cukup `git pull && docker compose up -d --build api`.
  - **Alur dev untuk kolom baru**: `alembic revision --autogenerate -m "..."` (draft — review `server_default`/nullable untuk baris lama) lalu `alembic upgrade head`; atau tulis migrasi manual. `E501` di `migrations/versions/` di-ignore ruff (gaya baris panjang bawaan Alembic).
  - **Test integrasi** (`tests/test_migrations.py`, 3 tes): fresh schema lengkap, upgrade idempoten, dan skenario DB lama parsial → konvergen + data lama utuh.
- **Konsekuensi:** rilis ke depan harus membawa migrasi (bukan hanya ubah model); `alembic upgrade head` butuh koneksi DB (di VPS sudah otomatis); DB yang sengaja dipakai tanpa migrasi (SQLite ad-hoc dev) perlu `alembic upgrade head` sekali (dokumentasi README). Catatan: backfill di migrasi awal menambahkan `server_default` pada kolom yang di model tidak punya default server — perbedaan DDL ini disengaja untuk mengisi baris lama.
