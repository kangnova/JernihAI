...
- **Validasi aktual cost model (Fase 1 — smoke test GPU pertama, Agustus 2026):** RTX 3060 12 GB di Vast.ai on-demand spot (`dph_total` $0,037/jam; T4 tidak tersedia di market saat itu). Hasil terukur: warm inference 4x 1080p **10,38 s** (KPI NFR-01 < 15 s ✅); encode WebP q90 8K **0,48 MB**; biaya sesi uji penuh (rent ~30 mnt + 5 iterasi + load model + encode) **±$0,02**; estimasi biaya GPU **per gambar ≈ $0,0001 → ±Rp 1,8/gambar** (@ Rp 16.500/USD) — **cost model Rp 2–6/gambar terkonfirmasi** (NFR-08). Revenue efektif ±Rp 500/gambar → gross margin GPU-only tetap > 99%; target ≥ 60% sangat aman. Sisa yang perlu dipantau di produksi: idle-time antar job (jangan biarkan instance nyala tanpa kerja) + egress R2.
